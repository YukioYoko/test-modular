    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm transition-all">
          ...
          <div className="relative w-full max-w-xl bg-(--mint-green) rounded-t-[2.5rem] p-6 shadow-2xl animate-slide-up h-[85vh] flex flex-col">
            
            {/* --- HEADER DEL CARRITO --- */}
            <div className="flex justify-between items-center mb-6 px-1">
              
              <div className="flex items-center gap-3">
                 <h2 className="text-2xl font-black text-(--militar-green) tracking-tight">Tu Pedido</h2>
              </div>
              ...
              <div className="flex items-center gap-2">
                 ...
                 >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-(--militar-green)">
                      <path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/>
                      <line x1="8" y1="8" x2="16" y2="8"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="8" y1="16" x2="10" y2="16"/>
                    </svg>
                    <span className="text-xs font-black text-(--militar-green)">CUENTA</span>
                 </Link>
                 ...
                 <button 
                  ...
                  >
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-(--militar-green) opacity-70"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                 </button>
              </div>
            </div>
              ...
              {items.length === 0 ? (
                ...
                items.map((item, index) => (
                  <div
                    ...
                    className="bg-white p-3 rounded-3xl flex gap-3 shadow-sm relative overflow-hidden group"
                  >
                    {/* IMAGEN DEL PRODUCTO */}
                      ...
                      <div className="flex justify-between items-end mt-2">
                        ...
                        <div className="flex items-center bg-slate-100 rounded-xl p-1 gap-2 h-8">
                          <button 
                            ...
                            className="w-6 h-full flex items-center justify-center bg-white rounded-lg shadow-sm text-(--militar-green) font-bold active:scale-90 transition-transform hover:bg-slate-50"
                          >
                            -
                          </button>
                          ...
                          <button 
                            ...
                            className="w-6 h-full flex items-center justify-center bg-(--militar-green) text-white rounded-lg shadow-sm font-bold active:scale-90 transition-transform hover:opacity-90"
                          >
                            +
                          </button>
                        </div>
            ...
            <div className="mt-auto bg-(--mint-green) pt-2 border-t border-(--militar-green)/10">
              <div className="flex justify-between px-4 items-center mb-4">
                <span className="font-bold text-(--militar-green) opacity-70">Total estimado</span>
              ...
              <button
                ...
                className={`w-full bg-white text-(--militar-green) py-5 rounded-3xl font-black text-lg shadow-lg active:scale-95 transition-transform flex justify-between px-8 items-center ${items.length === 0 ? "opacity-50 cursor-not-allowed" : "hover:shadow-xl"}`}
              >
                <span>{isPending ? "Enviando..." : `Confirmar Orden`}</span>
                <div className="bg-(--mint-green) p-1.5 rounded-full">
      ...
      {!isOpen && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md z-30 animate-in slide-in-from-bottom-4 duration-500">
          <button
            ...
            className="w-full bg-(--mint-green) h-16 rounded-2xl shadow-xl flex items-center justify-between px-6 transition-transform hover:scale-[1.02] active:scale-95 border border-white/20"
          >
            ...
            <div className="bg-(--color-not-white) p-2.5 rounded-xl text-(--militar-green) shadow-sm">
